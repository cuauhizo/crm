  ...
  <tr>
    ...
    <td class="whitespace-nowrap px-3 py-4 text-sm">
      ...
      @click="$emit('actualizar-estado', { id: cliente.id, estado: cliente.estado})"
        ...
        :class="[ estadoCliente ? 'bg-green-100 text-green-900' : 'bg-red-100 text-red-900']"
      >
        {{ estadoCliente ? "Activo" : "Inactivo" }}
      </button>
    </td>
    <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
      <RouterLink :to="{name:'editar-cliente', params: { id: cliente.id}}" class="text-indigo-600 hover:text-indigo-900 mr-5">
        Editar
      </RouterLink>
